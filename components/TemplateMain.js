import Image from "next/image";

export default function Main() {
  return (
    <section className="text-black body-font lg:pt-14">
      <div className="container px-5 mx-auto lg:px-4">
        <div className="flex flex-col w-full text-left md:text-center ">
          <h2 className="mb-1 text-5xl font-bold tracking-tighter text-center text-white lg:text-7xl md:text-6xl">
            NINE4 TEMPLATES
          </h2>
          <br></br>
          <p className="mx-auto text-xl font-normal leading-relaxed fs521 lg:w-2/3">
            Here is our collection of free to use templates made with Next.js &
            styled with Tailwind CSS.
          </p>
        </div>
        <div class="container my-12 mx-auto px-4 md:px-12">
          <div class="flex flex-wrap -mx-1 lg:-mx-4">
            <div class="my-1 px-1 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3">
              <div class="border border-gray-800 overflow-hidden rounded-lg shadow-lg">
                <a href="#">
                  <Image
                    alt="nine4-1"
                    className="block h-auto w-full"
                    src="https://nine4.app/images/nine4-1.png"
                    width={440}
                    height={270}
                  />
                </a>

                <div class="border-t border-gray-800 pt-3 pb-3 pl-3 space-y-4 bg-black text-c">
                  <div class="pl-1">
                    <a
                      href="https://github.com/r1/nine4-1"
                      class="text-3xl font-semibold text-white"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      nine4-1
                    </a>
                  </div>
                  <div class="text-xl font-medium">
                    <div class="text-white plc2">
                      <span class="ml-1 sdt sdt1"></span>
                      <a
                        class="pl-2"
                        href="https://nine4-1.vercel.app/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        Live Site
                      </a>
                    </div>
                    <div class="pl-2 text-white">
                      <a
                        class="inline-flex place-items-center"
                        href="https://github.com/r1/nine4-1/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <svg
                          width="14"
                          height="14"
                          viewBox="0 0 14 14"
                          aria-label="github"
                        >
                          <path
                            d="M7 .175c-3.872 0-7 3.128-7 7 0 3.084 2.013 5.71 4.79 6.65.35.066.482-.153.482-.328v-1.181c-1.947.415-2.363-.941-2.363-.941-.328-.81-.787-1.028-.787-1.028-.634-.438.044-.416.044-.416.7.044 1.071.722 1.071.722.635 1.072 1.641.766 2.035.59.066-.459.24-.765.437-.94-1.553-.175-3.193-.787-3.193-3.456 0-.766.262-1.378.721-1.881-.065-.175-.306-.897.066-1.86 0 0 .59-.197 1.925.722a6.754 6.754 0 0 1 1.75-.24c.59 0 1.203.087 1.75.24 1.335-.897 1.925-.722 1.925-.722.372.963.131 1.685.066 1.86.46.48.722 1.115.722 1.88 0 2.691-1.641 3.282-3.194 3.457.24.219.481.634.481 1.29v1.926c0 .197.131.415.481.328C11.988 12.884 14 10.259 14 7.175c0-3.872-3.128-7-7-7z"
                            fill="white"
                            fill-rule="nonzero"
                          ></path>
                        </svg>
                      </a>
                      <a
                        href="https://github.com/r1/nine4-1"
                        class="pl-2"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        Source Code
                      </a>
                    </div>
                    <div class="inline-flex px-5 py-1 mt-3 font-semibold text-white bg-gray-800 rounded-full toracking-wide">
                      Free
                    </div>
                  </div>
                </div>
              </div>
            </div>

            <div class="my-1 px-1 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3">
              <div class="border border-gray-800 overflow-hidden rounded-lg shadow-lg">
                <a href="#">
                  <Image
                    alt="nine4-2"
                    className="border border-gray-700 block h-auto w-full"
                    src="https://nine4.app/images/nine4-2.png"
                    width={440}
                    height={270}
                  />
                </a>

                <div class="border-t border-gray-800 pt-3 pb-3 pl-3 space-y-4 bg-black text-c">
                  <div class="pl-1">
                    <a
                      href="https://github.com/r1/nine4-1"
                      class="text-3xl font-semibold text-white"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      nine4-1
                    </a>
                  </div>
                  <div class="text-xl font-medium">
                    <div class="text-white plc2">
                      <span class="ml-1 sdt sdt1"></span>
                      <a
                        class="pl-2"
                        href="https://nine4-1.vercel.app/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        Live Site
                      </a>
                    </div>
                    <div class="pl-2 text-white">
                      <a
                        class="inline-flex place-items-center"
                        href="https://github.com/r1/nine4-1/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <svg
                          width="14"
                          height="14"
                          viewBox="0 0 14 14"
                          aria-label="github"
                        >
                          <path
                            d="M7 .175c-3.872 0-7 3.128-7 7 0 3.084 2.013 5.71 4.79 6.65.35.066.482-.153.482-.328v-1.181c-1.947.415-2.363-.941-2.363-.941-.328-.81-.787-1.028-.787-1.028-.634-.438.044-.416.044-.416.7.044 1.071.722 1.071.722.635 1.072 1.641.766 2.035.59.066-.459.24-.765.437-.94-1.553-.175-3.193-.787-3.193-3.456 0-.766.262-1.378.721-1.881-.065-.175-.306-.897.066-1.86 0 0 .59-.197 1.925.722a6.754 6.754 0 0 1 1.75-.24c.59 0 1.203.087 1.75.24 1.335-.897 1.925-.722 1.925-.722.372.963.131 1.685.066 1.86.46.48.722 1.115.722 1.88 0 2.691-1.641 3.282-3.194 3.457.24.219.481.634.481 1.29v1.926c0 .197.131.415.481.328C11.988 12.884 14 10.259 14 7.175c0-3.872-3.128-7-7-7z"
                            fill="white"
                            fill-rule="nonzero"
                          ></path>
                        </svg>
                      </a>
                      <a
                        href="https://github.com/r1/nine4-1"
                        class="pl-2"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        Source Code
                      </a>
                    </div>
                    <div class="inline-flex px-5 py-1 mt-3 font-semibold text-white bg-gray-800 rounded-full toracking-wide">
                      Free
                    </div>
                  </div>
                </div>
              </div>
            </div>

            <div class="my-1 px-1 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3">
              <div class="border border-gray-800 overflow-hidden rounded-lg shadow-lg">
                <a href="#">
                  <Image
                    alt="nine4-3"
                    class="block h-auto w-full"
                    src="https://nine4.app/images/nine4-3.png"
                    width={440}
                    height={270}
                  />
                </a>

                <div class="border-t border-gray-800 pt-3 pb-3 pl-3 space-y-4 bg-black text-c">
                  <div class="pl-1">
                    <a
                      href="https://github.com/r1/nine4-1"
                      class="text-3xl font-semibold text-white"
                      target="_blank"
                      rel="noopener noreferrer"
                    >
                      nine4-1
                    </a>
                  </div>
                  <div class="text-xl font-medium">
                    <div class="text-white plc2">
                      <span class="ml-1 sdt sdt1"></span>
                      <a
                        class="pl-2"
                        href="https://nine4-1.vercel.app/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        Live Site
                      </a>
                    </div>
                    <div class="pl-2 text-white">
                      <a
                        class="inline-flex place-items-center"
                        href="https://github.com/r1/nine4-1/"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        <svg
                          width="14"
                          height="14"
                          viewBox="0 0 14 14"
                          aria-label="github"
                        >
                          <path
                            d="M7 .175c-3.872 0-7 3.128-7 7 0 3.084 2.013 5.71 4.79 6.65.35.066.482-.153.482-.328v-1.181c-1.947.415-2.363-.941-2.363-.941-.328-.81-.787-1.028-.787-1.028-.634-.438.044-.416.044-.416.7.044 1.071.722 1.071.722.635 1.072 1.641.766 2.035.59.066-.459.24-.765.437-.94-1.553-.175-3.193-.787-3.193-3.456 0-.766.262-1.378.721-1.881-.065-.175-.306-.897.066-1.86 0 0 .59-.197 1.925.722a6.754 6.754 0 0 1 1.75-.24c.59 0 1.203.087 1.75.24 1.335-.897 1.925-.722 1.925-.722.372.963.131 1.685.066 1.86.46.48.722 1.115.722 1.88 0 2.691-1.641 3.282-3.194 3.457.24.219.481.634.481 1.29v1.926c0 .197.131.415.481.328C11.988 12.884 14 10.259 14 7.175c0-3.872-3.128-7-7-7z"
                            fill="white"
                            fill-rule="nonzero"
                          ></path>
                        </svg>
                      </a>
                      <a
                        href="https://github.com/r1/nine4-1"
                        class="pl-2"
                        target="_blank"
                        rel="noopener noreferrer"
                      >
                        Source Code
                      </a>
                    </div>
                    <div class="inline-flex px-5 py-1 mt-3 font-semibold text-white bg-gray-800 rounded-full toracking-wide">
                      Free
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}
